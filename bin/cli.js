...
async function buildIcons() {
  ...
  console.log(`✅ Cleaned and created output directory: ${outputDir}`);

  // 3. Find, read, and optimize all SVG files
  const svgFiles = await glob('**/*.svg', { cwd: inputDir });
  if (svgFiles.length === 0) {
    console.warn('⚠️ No SVG files found in the input directory. Exiting.');
    return;
  }
  ...
  console.log(`✅ Processed ${iconData.length} icons.`);
  ...
  if (mode === 'all' || mode === 'treeshakeable') {
    ...
    const { jsContent: utilityFunctions, dtsContent: utilityFunctionTypes } = generateUtilityFunctions();
  ...
  if (mode === 'all' || mode === 'full') {
    const { jsContent: helpersJsContent, dtsContent: helpersDtsContent } = generateUtilityFunctions();
  ...
  console.log(`✅ ${filesToWrite.length} files written successfully.`);
  console.log('🎉 Build complete! Your icon package is ready.');
}
